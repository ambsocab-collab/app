...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  // Configuración temporal para desarrollo sin Docker
  supabaseUrl = 'https://demo.supabase.co'
  supabaseServiceKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImV4cCI6MTk4MzgxMDk5Nn0.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU'
}
...
const isCircuitBreakerOpen = (): boolean => {
  ...
  if (circuitBreakerState.isOpen && now >= circuitBreakerState.nextAttemptTime) {
...
const recordCircuitBreakerFailure = (): void => {
  ...
  if (circuitBreakerState.failureCount >= circuitBreakerConfig.failureThreshold) {
    circuitBreakerState.isOpen = true
    circuitBreakerState.nextAttemptTime = Date.now() + circuitBreakerConfig.timeout
    logger.error(`Circuit breaker opened after ${circuitBreakerState.failureCount} failures`)
  }
}
  ...
  for (let attempt = 0; attempt < retryConfig.maxAttempts; attempt++) {
    try {
      ...
      if (attempt < retryConfig.maxAttempts - 1) {
        ...
        logger.warn(`${operationName} failed (attempt ${attempt + 1}/${retryConfig.maxAttempts}), retrying in ${delay}ms: ${lastError.message}`)
        await sleep(delay)
      } else {
        recordCircuitBreakerFailure()
        logger.error(`${operationName} failed after ${retryConfig.maxAttempts} attempts: ${lastError.message}`)
      }
    }
  }
...
export const checkDatabaseHealth = async (): Promise<{ status: 'healthy' | 'unhealthy', details?: any }> => {
  try {
    ...
    return {
      status: 'unhealthy',
      details: { error: (error as Error).message, circuitBreakerOpen: circuitBreakerState.isOpen }
    }
  }
}

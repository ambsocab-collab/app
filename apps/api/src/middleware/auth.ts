...
if (!JWT_SECRET) {
  console.warn('⚠️  Missing JWT_SECRET. Using development secret...')
  JWT_SECRET = 'development-secret-key-32-chars-minimum-length-required-for-jwt'
 ...
 */
export const generateAccessToken = (user: { id: string; email: string; name: string; role: string }): string => {
 ...
 */
export const generateRefreshToken = (user: { id: string; email: string; name: string; role: string }): string => {
  const payload: Omit<JWTPayload, 'type'> = {
    id: user.id,
    email: user.email,
    name: user.name,
 ...
 */
const validateUser = async (userId: string): Promise<{ id: string; email: string; name: string; role: string } | null> => {
  try {
    ...
    if (error || !data) {
      logger.warn(`User validation failed for user ${userId}: ${error?.message}`)
      return null
    }
 ...
 */
export const optionalAuthentication = async (req: AuthenticatedRequest, _res: Response, next: NextFunction): Promise<void> => {
  try {
    const token = extractTokenFromHeader(req.headers.authorization)
    ...
    try {
      ...
      logger.debug('Optional authentication failed:', error instanceof Error ? error.message : 'Unknown error')
    }
 ...
 */
export const validateRefreshToken = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const handleAuthError = (error: Error, res: Response): void => {
  ...
  } else if (error.message.includes('Invalid token') || error.message.includes('Token verification failed')) {
    res.status(401).json({
      success: false,
      error: 'Invalid Token',
 ...
 */
export const addPermissionsHeader = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (req.user) {
